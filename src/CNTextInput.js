...
class CNTextInput extends Component {
    ...
    addTextToContent(content, cursorPosition, textToAdd) {
        ...
        } else if (this.upComingStype !== null && startWithReadonly === false
            && this.upComingStype.link) {
            content = this.updateContent(content, {
                id: shortid.generate(),
                text: '',
                len: 0,
    ...
    createUpComingStyle(start, end, tag, stype) {
        this.upComingStype = {
            sel: {start, end},
            tag: tag,
            text: '',
            ...
            readOnly: false,
            stype: stype,
            styleList: StyleSheet.flatten(this.convertStyleList(update(stype, {$push: [tag, 'href']})))
        };
    }
    ...
    splitItems() {
        ...
        const result = this.findContentIndex(content, selectionEnd);
        let beforeContent = [];
        let afterContent = [];
